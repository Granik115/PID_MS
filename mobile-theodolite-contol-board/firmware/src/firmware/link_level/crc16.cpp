                                    ...
                                    uint16_t firstSymbolNumber)
{
  ...
  for (i = 0; i < lenght; ++i)
  {
    crc = (crc << 8)
    ^ crc16Table[(crc >> 8)
    ^ showSymbolFromRingBuffer(i + firstSymbolNumber, buf)];
  }
  return crc;
                        ...
                        uint16_t firstSymbolNumber)
{
  ...
  for (i = 0; i < lenght; ++i)
  {
    crc = (crc << 8) ^ crc16Table[(crc >> 8) ^ (buf->show(i + firstSymbolNumber) & 0xFF)];
  }
  return crc;
...
{
  uint16_t crc = 0;
  crc = (crc << 8) ^ crc16Table[(crc >> 8) ^ marker];
  crc = (crc << 8) ^ crc16Table[(crc >> 8) ^ len];
  while (len--)
  {
...
uint16_t crc16(uint_least8_t * pcBlock, uint32_t len)
{
  ...
  while (len--)
    crc = (crc << 8) ^ crc16Table[static_cast<uint_least8_t>((crc >> 8) ^ *pcBlock++)];
...
{
  uint16_t crc = 0;
  volatile uint32_t testLen = len;
  while (testLen--)
  {
    const uint16_t word = *pcBlock++;
    crc = (crc << 8) ^ crc16Table[static_cast<uint_least8_t>((crc >> 8) ^ (word >> 8))];
    crc = (crc << 8) ^ crc16Table[static_cast<uint_least8_t>((crc >> 8) ^ (word & 0xFF))];
  }
  return crc;
}
