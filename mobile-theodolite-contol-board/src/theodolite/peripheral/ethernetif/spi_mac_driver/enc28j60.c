...
#include "theodolite/peripheral/ethernetif/spi_mac_driver/debug.h"
#include "pbuf.h"
#include "netif.h"
 ...
 * @return Error code
 **/
static void enc28j60WriteBufferPbuf(NetInterface *interface, const struct pbuf *p, size_t offset);
static struct pbuf* enc28j60ReceivePacketPbuf(struct netif *netif);
static void enc28j60ReadBufferToPbuf(NetInterface *interface, struct pbuf *p, size_t length);

extern void nicProcessPacket(NetInterface *interface, uint8_t *packet, size_t length,
...
error_t enc28j60Init(NetInterface *interface)
{
  ...
  uint8_t revisionId = enc28j60ReadReg(interface, ENC28J60_EREVID);
  ...
  volatile uint8_t mcaddr = enc28j60ReadReg(interface, ENC28J60_MAADR4);
  ...
  //enc28j60WritePhyReg(interface, ENC28J60_PHLCON,
  //ENC28J60_PHLCON_LACFG_BLINK_FAST | ENC28J60_PHLCON_LBCFG_BLINK_FAST);
...
osStatus_t enc28j60IrqHandler(NetInterface *interface)
{
  return osSemaphoreRelease(interface->irq_event_semaphore);
}
...
bool_t enc28j60IrqHandler_Old(NetInterface *interface)
{
  bool_t flag;
  uint8_t status;
  ...
  if ((status & ENC28J60_EIR_LINKIF) != 0)
  {
    ...
    interface->nicEvent = TRUE;
    //Notify the TCP/IP stack of the event
    flag |= osSetEventFromIsr(&interface->netEvent);
  }
...
void enc28j60EventHandler(NetInterface *interface)
{
  ...
  if (enc28j60ReadReg(interface, ENC28J60_EPKTCNT) != 0)
  {
    ...
    struct pbuf *p = NULL;
    struct netif *netif = interface->ipStackInterface;
    //Process all pending packets
    do
    {
      p = enc28j60ReceivePacketPbuf(netif);
      if (p != NULL)
      {
        const err_t err = netif->input(p, netif);
        if (err != ERR_OK)
        {
          NETIF_DEBUGF(NETIF_DEBUG_WARNING,
          ("netif input error: %d on %c%c\n", err, netif->name[0], netif->name[1]));
          pbuf_free(p);
        }
      }
    } while (p != NULL);
  ...
  if ((status & (ENC28J60_EIR_TXIF | ENC28J60_EIE_TXERIE)) != 0)
  {
    ...
    //Notify  that the transmitter is ready to send
    osSemaphoreRelease(interface->tx_ready_semaphore);
  }
...
error_t enc28j60SendPacketPbuf(NetInterface *interface,
                               const struct pbuf *buffer,
                               size_t offset)
{
  size_t length;

  //Retrieve the length of the packet
  length = buffer->tot_len - offset;
  ...
  enc28j60WriteBufferPbuf(interface, buffer, offset);
...
static struct pbuf* enc28j60ReceivePacketPbuf(struct netif *netif)
{
  ...
  if (enc28j60ReadReg(interface, ENC28J60_EPKTCNT) != 0)
  {
    ...
    if ((status & ENC28J60_RSV_RECEIVED_OK) != 0)
    {
      ...
      if (p != NULL)
      {
        enc28j60ReadBufferToPbuf(interface, p, length);
        error = NO_ERROR;
      }
      else
      {
        NETIF_DEBUGF(NETIF_DEBUG_WARNING,
        ("Out of mem %c%c\n", interface->name[0], interface->name[1]));
...
static void enc28j60WriteBufferPbuf(NetInterface *interface, const struct pbuf *p, size_t offset)
{
  ...
  for (q = p; q != NULL; q = q->next)
    interface->spiDriver->spiTransferDma(q->payload, q->len, TX_SPI_DIRECTION);
  //Terminate the operation by raising the CS pin
  interface->spiDriver->deassertCs();
}
...
static void enc28j60ReadBufferToPbuf(NetInterface *interface, struct pbuf *p, size_t length)
{
  ...
  struct pbuf *q;
  for (q = p; q != NULL; q = q->next)
    interface->spiDriver->spiTransferDma(q->payload, q->len, RX_SPI_DIRECTION);
//Terminate the operation by raising the CS pin
  interface->spiDriver->deassertCs();
}
...
void enc28j60Start(NetInterface *interface)
{
  interface->extIntDriver->enableIrq();
}

void enc28j60SendPacketThread(void *argument)
{
  struct pbuf *p = NULL;
  ...
  for (;;)
  {
    if (osMessageQueueGet(interface->tx_message_queue, (void*)&p, 0,
    osWaitForever) == osOK)
    {
      if (p != NULL)
      {
        const osStatus_t status = osSemaphoreAcquire(
                                                     interface->tx_ready_semaphore,
                                                     5000);
        if (status != osOK)
        {
          assert(status == osOK);
        ...
        const error_t err = enc28j60SendPacketPbuf(interface, p, 0);
        if (err != NO_ERROR)
          osSemaphoreRelease(interface->tx_ready_semaphore);
      ...
      {
        LWIP_DEBUGF(NETIF_DEBUG_WARNING, ("NULL pbuf\n"));
      }
    }
    else
    {
      LWIP_DEBUGF(NETIF_DEBUG_WARNING, ("tx message error\n"));
    }
  }
}

void enc28j60EventsThread(void *argument)
{
  struct netif *netif = (struct netif*)argument;
  NetInterface *interface = (NetInterface*)netif->state;
  ...
  for (;;)
  {
    osStatus_t status = osSemaphoreAcquire(interface->irq_event_semaphore,
                                           200);
    if ((status == osOK) || (status == osErrorTimeout))
    {
      osMutexAcquire(interface->mutex, osWaitForever);
      enc28j60EventHandler(interface);
      osMutexRelease(interface->mutex);
    }
  }
}
